...
export const TIER_THRESHOLDS = {
  GOLD: 25000,      // ₹25,000 lifetime spend to reach Gold
  PLATINUM: 100000  // ₹100,000 lifetime spend to reach Platinum
};

export interface TierBenefit {
  tier: Tier;
  earnMultiplier: number;  // Points multiplier (e.g., 1.5x for Gold)
  perks: string[];
}

export const TIER_BENEFITS: Record<Tier, TierBenefit> = {
  [Tier.MEMBER]: {
    tier: Tier.MEMBER,
    earnMultiplier: 1.0,
...
export interface LoyaltyConfig {
  defaultRate: number; // Percentage (e.g. 10)
  categoryRates: Partial<Record<TransactionCategory, number>>;
  redemptionRate: number; // Points per currency unit (e.g. 1 pt = 1 INR)
}
...
export interface RetentionMetrics {
  ...
  ltv: number;
  churnRisk: number; // %
  topFamilies: { name: string; spend: number }[];
}
