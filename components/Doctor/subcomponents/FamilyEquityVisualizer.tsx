...
const FamilyEquityVisualizer: React.FC<Props> = ({ patient, allUsers, familyGroups, primaryColor }) => {
    ...
    return (
        <div className="p-8 bg-dark-900 rounded-[24px] text-white space-y-8 relative overflow-hidden glass-panel border-0">
            <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 blur-[50px] rounded-full"></div>
            <div className="flex justify-between items-center relative z-10">
                <div>
                    <h4 className="text-xl font-bold tracking-tight">{family.familyName}</h4>
                    <p className="text-[10px] font-bold uppercase text-ink-500 tracking-widest mt-1">Shared Household Equity</p>
                </div>
            ...
            <div className="pt-6 border-t border-white/10 relative z-10">
                <div className="flex justify-between items-end">
                    <div>
                        <p className="text-[9px] font-bold text-ink-500 uppercase tracking-widest mb-1">Total Pool Strength</p>
                        <h5 className="text-3xl font-bold">₹{members.reduce((a, b) => a + b.lifetimeSpend, 0).toLocaleString()}</h5>
                    </div>
                    <div className="text-right">
                        <span className="text-[9px] font-bold uppercase bg-emerald-500/10 text-emerald-400 px-3 py-1 rounded-full border border-emerald-500/20">Sync Active</span>
                    </div>
                </div>
            </div>
        </div>
