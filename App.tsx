...
const App = () => {
    ...
    return (
      <div className="h-[100dvh] w-full flex items-center justify-center bg-slate-950">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-500 font-mono text-xs uppercase tracking-widest animate-pulse">Initializing Retain Dental...</p>
        </div>
      </div>
    );
  }
  ...
  const handlers = {
    ...
    onHardDeleteUser: async (userId: string) => {
      ...
      if (result.success && result.updatedData) {
        setData(prev => ({ ...prev, ...result.updatedData }));
        addToast("Identity Obliterated", "success");
      } else {
        addToast(`Deletion Failed: ${result.message}`, "error");
      }
      return result;
    }
  };
