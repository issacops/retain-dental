    ...
    <div className="relative mx-auto w-[280px] sm:w-[300px]">
      <div className="rounded-[2.5rem] bg-gradient-to-b from-[#fdf8f0] to-[#f0e8db] border border-gray-200 shadow-2xl shadow-teal-900/10 overflow-hidden">
        ...
        <div className="px-5 py-2 flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-violet-600 flex items-center justify-center text-white text-[9px] font-semibold">R</div>
          <span className="text-xs font-semibold text-onDark-100">RetainOS Clinic</span>
        </div>
        ...
        <div className="px-5 pb-3 min-h-[320px]">
          {activeTab === 'home' && (
            <div className="space-y-3">
              {/* Loyalty card */}
              <div className="bg-ink-900 rounded p-4 shadow-sm border border-ink-border">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <p className="text-[9px] font-semibold text-onDark-500 uppercase tracking-widest">Loyalty Status</p>
                    <p className="text-sm font-semibold text-onDark-100 mt-0.5">Welcome, Sarah</p>
                  </div>
                  <div className="px-2.5 py-1 bg-ink-800 rounded">
                    <span className="text-[9px] font-semibold text-violet-600">GOLD</span>
                  </div>
                </div>
              ...
              {/* Next appointment */}
              <div className="bg-ink-900 rounded p-4 shadow-sm border border-ink-border">
                <p className="text-[9px] font-semibold text-onDark-500 uppercase tracking-widest mb-2">Next Visit</p>
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded bg-ink-800 flex flex-col items-center justify-center shrink-0">
                    <span className="text-[9px] font-semibold text-violet-600 leading-none">JAN</span>
                    <span className="text-lg font-semibold text-violet-600 leading-none mt-0.5">15</span>
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-onDark-100">Cleaning & Checkup</p>
          ...
          {activeTab === 'wallet' && (
            <div className="space-y-3">
              <div className="bg-ink-950 rounded p-4 text-white">
                <p className="text-[9px] font-semibold text-white/50 uppercase tracking-widest">Balance</p>
                <p className="text-3xl font-semibold mt-1">2,450</p>
                <p className="text-xs text-white/60">Smile Points</p>
              </div>
              {walletItems.map((item, i) => (
                <div key={i} className="bg-ink-900 rounded p-3 shadow-sm border border-ink-border flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-semibold text-onDark-100">{item.label}</p>
                      <p className="text-[9px] text-onDark-500">{item.time}</p>
                    </div>
                  </div>
            ...
            <div className="space-y-3">
              <div className="bg-ink-900 rounded p-4 shadow-sm border border-ink-border">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-2 h-2 rounded bg-emerald-500 animate-pulse"></div>
                  <p className="text-xs font-semibold text-onDark-100">Daily Care Ritual</p>
                </div>
                  ...
                  {steps.map((s, i) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 p-2.5 rounded transition-all ${
                        s.done ? 'bg-emerald-50 border border-emerald-200' : 'bg-ink-800 border border-ink-border'
                      }`}
                    >
                      <div className={`w-5 h-5 rounded flex items-center justify-center shrink-0 ${
                        s.done ? 'bg-emerald-500 text-white' : 'border-2 border-onDark-500'
                      }`}>
                        {s.done && <span className="text-[8px]">✓</span>}
                      </div>
                      <span className={`text-xs font-semibold ${s.done ? 'text-emerald-700 line-through' : 'text-onDark-500'}`}>
                        {s.title}
                      </span>
                    </button>
                  ))}
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span className="text-[9px] text-onDark-500">{steps.filter(s => s.done).length}/{steps.length} complete</span>
          ...
          {activeTab === 'profile' && (
            <div className="space-y-3">
              ...
              <div className="bg-ink-900 rounded p-4 shadow-sm border border-ink-border">
                <p className="text-[9px] font-semibold text-onDark-500 uppercase tracking-widest mb-2">Household</p>
                <div className="space-y-2">
                  {['Sarah (You)', 'Mike Johnson', 'Emma Johnson'].map((name, i) => (
                    <div key={i} className="flex items-center gap-2.5 p-2 rounded bg-ink-800">
                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-white text-[8px] font-semibold ${i === 0 ? 'bg-violet-600' : i === 1 ? 'bg-amber-500' : 'bg-ink-800'}`}>
                        {name[0]}
                      </div>
                      <span className="text-xs font-semibold text-onDark-500">{name}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="px-4 pb-4">
          <div className="bg-ink-900/95 backdrop-blur-xl rounded-[28px] p-1.5 flex shadow-lg border border-ink-border">
            {tabs.map(tab => (
              <button
                key={tab.id}
