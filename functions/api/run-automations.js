...
/**
 * Cloudflare Pages Function — scheduled automations.
 *
 * Trigger via a Cloudflare Cron Trigger or any external scheduler hitting
 * POST/GET /api/run-automations. Safe to re-run (dedupe_key prevents doubles).
 *
 * Rules come from clinics.settings.automations:
 *   appointmentReminder · recallNudge · birthday · festive
 */
const FESTIVE = [
...
export async function onRequest(context) {
  ...
  if (!supabaseUrl || !serviceRoleKey) return json({ error: 'Supabase service credentials missing' }, 500);
  ...
  try {
    const { data: clinics, error } = await supabase.from('clinics').select('id, name, settings');
    ...
    for (const clinic of clinics || []) {
      ...
      if (auto.appointmentReminder?.enabled) {
        ...
        for (const a of appts || []) {
          ...
          rows.push({
            clinic_id: clinic.id, patient_id: a.patient_id,
            ...
            category: 'Appointment', status: 'SENT', sent_at: now.toISOString(), sent_by: 'Automation',
      ...
      if (auto.recallNudge?.enabled) {
        ...
        const { data: profiles } = await supabase.from('profiles').select('id, full_name').eq('clinic_id', clinic.id).eq('role', 'PATIENT');
        ...
        if (patientIds.length) {
          ...
          const { data: earns } = await supabase.from('transactions').select('wallet_id, created_at').eq('clinic_id', clinic.id).eq('type', 'EARN');
          ...
          for (const p of profiles || []) {
            ...
            rows.push({
              clinic_id: clinic.id, patient_id: p.id,
              ...
              category: 'Recall', status: 'SENT', sent_at: now.toISOString(), sent_by: 'Automation',
      ...
      if (auto.birthday?.enabled || auto.festive?.enabled) {
        const { data: profiles } = await supabase.from('profiles').select('id, full_name, metadata').eq('clinic_id', clinic.id).eq('role', 'PATIENT');
        for (const p of profiles || []) {
          ...
          if (auto.birthday?.enabled) {
            ...
            if (dob && String(dob).slice(5, 10) === todayMd) {
              rows.push({
                ...
                category: 'Greeting', status: 'SENT', sent_at: now.toISOString(), sent_by: 'Automation',
    ...
    return json({ error: e.message || 'Automation run failed', ...summary }, 500);
  }
}
